...
teks_komentar = st.text_area("Please input the comment:")
file_excel = st.file_uploader("Or upload a file with comments (Excel):", type=["xlsx"])
...
if st.button("Prediksi"):
    if teks_komentar:
        ...
        st.write("Hasil Prediksi:", "Toxic" if prediksi[0][0] > 0.5 else "Non-toxic")
    ...
    if file_excel is not None:
        ...
        df['Prediksi'] = np.where(prediksi_df.flatten() > 0.5, "Toxic", "Non-toxic")
        st.write(df[['kolom_komentar', 'Prediksi']])
        st.write("Time Prediction: {:.2f} second".format(elapsed_time))
